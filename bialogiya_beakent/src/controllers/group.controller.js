...
const createGroup = async (req, res, next) => {
  try {
    const { name, description, subject, icon, color, teacherId } = req.body;
    if (!name) return error(res, 'Group name required', 400);

    const targetTeacherId = teacherId || req.user.userId;
    const teacher = await prisma.user.findUnique({ where: { id: targetTeacherId, role: 'teacher', isActive: true } });
    if (!teacher) return error(res, 'Teacher not found', 404);

    const group = await prisma.group.create({
      data: { name, description, subject: subject || 'biology', icon, color, teacherId: targetTeacherId },
...
const getAllGroups = async (req, res, next) => {
  try {
    const where = req.user.role === 'admin' || req.user.role === 'reception' ? {} : { teacherId: req.user.userId };
    const groups = await prisma.group.findMany({
...
const updateGroup = async (req, res, next) => {
  try {
    const { name, description, subject, icon, color, isActive, teacherId } = req.body;
    const updateData = { name, description, subject, icon, color, isActive };
    if (teacherId) {
      const teacher = await prisma.user.findUnique({ where: { id: teacherId, role: 'teacher', isActive: true } });
      if (teacher) updateData.teacherId = teacherId;
    }
    const group = await prisma.group.update({ where: { id: req.params.id }, data: updateData });
    return success(res, group);
  } catch (err) { next(err); }
};
