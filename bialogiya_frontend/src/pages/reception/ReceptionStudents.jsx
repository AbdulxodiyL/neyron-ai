...
import { Plus, Copy, RefreshCw, X, GraduationCap, Phone, Users, Star, Zap, Pencil, Trash2, Save, ChevronRight, UserCheck } from 'lucide-react';
...
export default function ReceptionStudents() {
  ...
  const { data: students } = useQuery({
    ...
    queryFn: () => api.get('/reception/students').then(r => r.data.data),
  });
  ...
  const isLoading = !students || !groups;

  const createMutation = useMutation({
    mutationFn: (d) => api.post('/reception/students', d),
    onSuccess: ({ data }) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => api.put(`/reception/students/${id}`, data),
    onSuccess: ({ data }) => {
      qc.invalidateQueries(['reception-students']);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/reception/students/${id}`),
    onSuccess: () => {
      qc.invalidateQueries(['reception-students']);
  ...
  const freezeMutation = useMutation({
    mutationFn: (id) => api.patch(`/reception/students/${id}/freeze`),
    onSuccess: ({ data }) => {
      qc.invalidateQueries(['reception-students']);
  ...
  const resetPwMutation = useMutation({
    mutationFn: (id) => api.post(`/reception/students/${id}/reset-password`),
    onSuccess: ({ data }) => {
      ...
      <div className="space-y-2">
        ...
        {students?.length === 0 && (
          <div className="text-center py-16 text-gray-400">
            ...
            <p>Bu guruhda o'quvchi yo'q. Birinchi o'quvchini qo'shing!</p>
          </div>
        )}
      </div>
            ...
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => { if (window.confirm(`"${selectedStudent.name}" ni o'chirishni tasdiqlaysizmi?`)) deleteMutation.mutate(selectedStudent.id); }}
              ...
              <div className="flex flex-col gap-2">
                <button
                  onClick={() => { if (window.confirm('Parolni yangilash?')) resetPwMutation.mutate(selectedStudent.id); }}
                ...
                <button
                  onClick={() => freezeMutation.mutate(selectedStudent.id)}
                  disabled={freezeMutation.isPending}
                  className="btn-ghost w-full flex items-center justify-center gap-2 text-sm border border-gray-200 dark:border-gray-700">
                  {selectedStudent.isFrozen ? 'Muzlatdan chiqarish' : 'Muzlatish'}
                </button>
              </div>
            </motion.div>
          </motion.div>
